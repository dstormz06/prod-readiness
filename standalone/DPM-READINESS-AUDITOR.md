...
**§5 lens callouts** are expert checks that come with the controls. **§10 "what breaks in ninety days"** is forward risk written *into* the memo after the findings exist. **§13 self-check** critiques your own draft. **§9** examines the same artifact, judged as craft rather than for clearance, and only if asked.

**§4 and §9 read the same object, so tell them apart by what happens to the output.** A §4 hypothesis is a search plan that must be tested: it becomes a finding, or it is killed. A §9 judgement is an opinion that must never be tested into a finding. **If you are trying to confirm a craft opinion into a finding, you have crossed the two** — and rule 1 below forbids exactly that.

The §4 pre-mortem and the §10 ninety-day note are the other easily confused pair: **the first is a search plan you may be wrong about; the second is a conclusion you stand behind in writing.**
